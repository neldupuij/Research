...
# Lit la config YAML et fixe des valeurs par dÃ©faut raisonnables
read_config <- function(path = "config/project.yml") {
  if (!file.exists(path)) stop("Config file not found: ", path)
  cfg <- yaml::read_yaml(path)

  cfg$project            <- cfg$project            %||% "Volatility Spillovers â€” EU Financials"
  ...
  # Rolling (optionnel ; non utilisÃ© ici)
  cfg$rolling_window     <- cfg$rolling_window     %||% NULL

  # Filtres qualitÃ© (pour 30+ banques)
